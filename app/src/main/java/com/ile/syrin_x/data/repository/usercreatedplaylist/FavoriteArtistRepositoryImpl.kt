...
class FavoriteArtistRepositoryImpl @Inject constructor(
    ...
    override suspend fun getAllFavoriteArtistsByUser(userId: String): Flow<Response<List<FavoriteArtist>>> = callbackFlow {
        trySend(Response.Loading)

        val localJob = launch {
            favoriteArtistDao.getAllForUser(userId)
                .map { entities ->
                    entities.map { e ->
                        FavoriteArtist(
                            id = e.id,
        ...
        val listener = object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                launch {
                    val entities = snapshot.children.mapNotNull { it.getValue(FavoriteArtistEntity::class.java) }
